...
export function buildOrderMessage(service, durationKey, priceQar, lang) {
  const durationEn = durationKey === "month" ? "1 Month" : "1 Year";
  const durationAr = durationKey === "month" ? "شهر واحد" : "سنة واحدة";
  if (lang === "ar") {
    return `مرحباً فريق دعم Social OTT - Qatar، أود شراء الاشتراك التالي:
...
يرجى تزويدي بتفاصيل الدفع وإتمام طلبي.`;
  ...
  return `Hello Social OTT - Qatar Support Team, I would like to purchase the following subscription:
